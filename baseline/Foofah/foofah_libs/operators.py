...
def f_inner_join_tables(table1, table2):
    # Join two tables, table1 and table2, with column indexes p1 of table1 and p2 of table2
    table1_rows = []
    for row in table1:
        table1_rows.append(ast.literal_eval(row))
    table2_rows = []
    for row in table2:
        table2_rows.append(ast.literal_eval(row))

    # Auto-Pipeline: Join operator is also known as merge in pandas
    df1 = pd.DataFrame(table1_rows[1:], columns=table1_rows[0])
    df1Cols = df1.columns.tolist()
    df2 = pd.DataFrame(table2_rows[1:], columns=table2_rows[0])
    
    df2Cols = df2.columns.tolist()
    overlapCols = list(set(df1Cols).intersection(set(df2Cols)))
    if not overlapCols: return None
    new_table = []
    try:
        joinDf = df1.merge(df2, how='inner', left_on=overlapCols, right_on=overlapCols).drop_duplicates()
        if joinDf.empty: return None
        new_table.append("['" + "', '".join(str(colName) for colName in joinDf.columns) + "']")
        for row in joinDf.values.tolist():
            new_table.append("['" + "', '".join(str(val) for val in row) + "']")
    except:
        return None
    return new_table

def f_left_join_tables(table1, table2):
    # Join two tables, table1 and table2, with column indexes p1 of table1 and p2 of table2
    table1_rows = []
    for row in table1:
        table1_rows.append(ast.literal_eval(row))
    ...
    new_table = []
    try:
        joinDf = df1.merge(df2, how='left', left_on=overlapCols, right_on=overlapCols).drop_duplicates()
        if joinDf.empty: return None
        new_table.append("['" + "', '".join(str(colName) for colName in joinDf.columns) + "']")
        for row in joinDf.values.tolist():
            new_table.append("['" + "', '".join(str(val) for val in row) + "']")
...
def f_outer_join_tables(table1, table2):
    # Join two tables, table1 and table2, with column indexes p1 of table1 and p2 of table2
    table1_rows = []
    for row in table1:
        table1_rows.append(ast.literal_eval(row))
    ...
    new_table = []
    try:
        joinDf = df1.merge(df2, how='outer', left_on=overlapCols, right_on=overlapCols).drop_duplicates()
        if joinDf.empty: return None
        new_table.append("['" + "', '".join(str(colName) for colName in joinDf.columns) + "']")
        for row in joinDf.values.tolist():
            new_table.append("['" + "', '".join(str(val) for val in row) + "']")
...
def f_union_tables(table1, table2):
    # Join two tables, table1 and table2, with column indexes p1 of table1 and p2 of table2
    table1_rows = []
    for row in table1:
        table1_rows.append(ast.literal_eval(row))
    ...
    if len(df1Cols) != len(df2Cols): return None
    similarCols = set()
    for col in df1Cols:
        if col in df2Cols: similarCols.add(col)
        if True in [col in col2 for col2 in df2Cols]: similarCols.add(col)
    if len(similarCols) == len(df1Cols) and len(similarCols) == len(df2Cols):
        new_table = []
        try:            
            unionDf = pd.concat([df1, df2], ignore_index=True).drop_duplicates()
            new_table.append("['" + "', '".join(str(colName) for colName in unionDf.columns) + "']")
            for row in unionDf.values.tolist():
                new_table.append("['" + "', '".join(str(val) for val in row) + "']")
        except:
            return None
        return new_table
...
def add_ops_tables():
    ops = list()
    ops.append(
        {'name': 'f_inner_join_tables', 'fxn': lambda x1, x2: f_inner_join_tables(x1, x2), 'params': {}, 'if_col': True, 'char': '', 'cost': 1.0,
         'num_params': 2,
         })
    ops.append(
        {'name': 'f_left_join_tables', 'fxn': lambda x1, x2: f_left_join_tables(x1, x2), 'params': {}, 'if_col': True, 'char': '', 'cost': 1.0,
         'num_params': 2,
         })
    ops.append(
        {'name': 'f_outer_join_tables', 'fxn': lambda x1, x2: f_outer_join_tables(x1, x2), 'params': {}, 'if_col': True, 'char': '', 'cost': 1.0,
         'num_params': 2,
         })
    ops.append(
        {'name': 'f_union_tables', 'fxn': lambda x1, x2: f_union_tables(x1, x2), 'params': {}, 'if_col': True, 'char': '', 'cost': 1.0,
         'num_params': 2,
         })
    return ops
