...
class FoofahNode:
    ...
    # @profile
    def make_children(self, tableQueue, ops, debug=False, bound=float("inf"), p1=True, p2=True, p3=True):
        start = timer()
        num_cols = len(ast.literal_eval(self.contents[0]))
        ...
        ops = temp_ops
        temp_tables = list(tableQueue.queue)
        opTables = temp_tables
        ...
        for i, opT in enumerate(opTables):
            numOpT_Cols = len(ast.literal_eval(opT.contents[0]))            
            for op in ops:
                result = op['fxn'](self.contents, opT.contents)
                ...
                if child:
                    print("column parameter child: ", child, opT.node_id)
                    children.append(child)
                    tablesOpedOn.add(opT)

        self.times['children'].append(timer() - start)

        return children, list(tablesOpedOn)
    ...
    def make_child_node(self, table, parent_node, op_obj, p1=True, p2=True, p3=True):
        ...
        if p3 and child.identical(parent_node):
            return None
    ...
    def get_h_score_modified(self):
        tableCols = ast.literal_eval(self.contents[0])   
        goalCols = FoofahNode.goal_node.contents[0]
        colOverlap = [col for col in tableCols if col in goalCols]
        valOverlap = []
        numValOverlaps = 0
        for col in colOverlap:
            goalColIndx = goalCols.index(col)
            goalColVals = [row[goalColIndx] for row in FoofahNode.goal_node.contents[1:] if row[goalColIndx]]
            tableColIndx = tableCols.index(col)
            tableColVals = [ast.literal_eval(row)[tableColIndx] for row in self.contents[1:] if row[tableColIndx]]
            overlapVals = set(goalColVals).intersection(set(tableColVals))
            valOverlap.append(len(overlapVals)/len(set(goalColVals)))
        if valOverlap: numValOverlaps = sum(valOverlap)/len(valOverlap)
        return 1-numValOverlaps
        
        
    def get_any_dist(self, node_a, node_b, batch=True):
        if CPP:
            # a = foofah_utils.TableGrapph(node_a.num_rows, node_a.num_cols)
